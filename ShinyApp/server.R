...
## app.R ##
 ...
 library(Biobase)
 library(ggplot2)
# source("/app/Shiny/RAExpressionDashboard/utils.R")
# source("HPAplots.R")
# # res <- Gtex[,1:2]
...
server <- function(input, output)  { 
  ...
  library(ggplot2)
  source("/app/Shiny/RAExpressionDashboard/utils.R")
  source("/app/Shiny/RAExpressionDashboard/HPAplots.R")
  # res <- Gtex[,1:2]
  # res <- res[(!duplicated(res$Description)),]
  # colnames(res) <- c("Gene_Name", "Gene_id")
  load("/app/Shiny/RAExpressionDashboard/Data/res1.rData")
  ...
  output$dataset1 <- renderTree({ 
    sss= list( 
      'Cell/Tissue Expression'   =  structure(list("Fantom5" = "f5",
                                                   "GTEX" = "gtex",
                                                   "SingleCellHPA" = "SingleCellHPA"),stselected=FALSE),  
      ...
      'Disease Expression'     =  structure(list('AMP'='AMP', 
                                                 ...
                                                 "GSE116899" = "GSE116899", 
                                                 "GSE153015" = "GSE153015",
                                                 "GSE74143" = "GSE74143"), stselected=FALSE),
      'Treatment Effects'      = structure(list("Recombine" ="recombine",
                                                "GSE24742" = "GSE24742",
                                                "GSE45867" = "GSE45867",
                                                "GSE47726" = "GSE47726"), stselected=FALSE)
    ) 
  })
  ...
  output$boxes <- renderUI({
    tagList(
      ...
      if("GSE153015" %in% dataset()){
        box(title = "GSE153015",status = "primary", solidHeader = TRUE,
            plotOutput("plot19"), width=12)
      },
      
      if("SingleCellHPA" %in% dataset()){
        box(title = "SingleCellHPA",status = "primary", solidHeader = TRUE,
            plotOutput("plot18"), width=12)
      },
      
      if("GSE47726" %in% dataset()){
        box(title = "GSE47726",status = "primary", solidHeader = TRUE,
            plotOutput("plot20"), width=12)
      },
      
      if("GSE74143" %in% dataset()){
        box(title = "GSE74143",status = "primary", solidHeader = TRUE,
            plotOutput("plot21"), width=12)
      }
  ...
  output$plot1 <- renderPlot({
    ...
    PlotFantonFunction_old(gene=SW())
  })
  ...
  output$plot18 <- renderPlot({
    SingleCelllot(gene=SW())
  }) 
  output$plot19 <- renderPlot({
    PlotGSE153015(gene=SW())
  }) 
  output$plot20 <- renderPlot({
    PlotGSE47726(gene=SW())
  }) 
  output$plot21 <- renderPlot({
    PlotGSE74143(gene=SW())
  }) 
